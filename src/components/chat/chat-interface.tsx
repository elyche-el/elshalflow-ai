...
export function ChatInterface() {
  ...
  useEffect(() => {
    ...
    fetch("/api/models?key=" + encodeURIComponent(k || ""))
      ...
      .then((d: any) => { if (Array.isArray(d) && d.length) setModels(d.map((m: any): ModelInfo => ({ id: m.id, name: m.name, is_free: m.is_free, provider: m.top_provider?.name || "", pricing: m.pricing?.prompt || "0", multimodal: (m.architecture?.modality || "").includes("image") }))); })
      .catch(() => {});
  }, []);
  ...
  function hFiles(files: FileList) {
    ...
    for (let i = 0; i < files.length; i++) {
      if (!files[i].type.startsWith("image/")) { toast.error(files[i].name + " non image"); continue; }
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    const b64s = await Promise.all(images.map((i: any) => toB64(i.file)));
    const prvs = images.map((i: any) => i.preview);
    let c: any = input || "[Image]";
    if (b64s.length) { const parts: any[] = []; if (input.trim()) parts.push({ type: "text", text: input }); b64s.forEach((b: string) => parts.push({ type: "image_url", image_url: { url: b } })); c = parts; }
    const um: Message = { id: "u" + Date.now(), role: "user", content: input.trim() || "[Image]", images: prvs, model };
    setMsgs((p: Message[]) => [...p, um]); setInput(""); setImages([]); setLoading(true);
    const ams = msgs.map((m: Message) => ({ role: m.role, content: m.images?.length ? m.content || "[Image]" : m.content }));
    ams.push({ role: "user", content: c } as any);
    const key = getKey();
    ar.current = new AbortController();
    const aid = "a" + Date.now();
    setMsgs((p: Message[]) => [...p, { id: aid, role: "assistant", content: "", model }]);
    try {
      ...
      while (true) {
        ...
        setMsgs((p: Message[]) => p.map((m: Message) => m.id === aid ? { ...m, content: full } : m));
      }
    } catch (err: any) {
      if (err.name !== "AbortError") { toast.error(err.message); setMsgs((p: Message[]) => p.map((m: Message) => m.id === aid ? { ...m, content: "Erreur: " + err.message } : m)); }
  ...
  const frees = models.filter((m: ModelInfo) => m.is_free).slice(0, 8);
  const paids = models.filter((m: ModelInfo) => !m.is_free).slice(0, 8);
