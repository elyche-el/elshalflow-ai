...
const MODELS = [
  ...
  { id: "nvidia/nemotron-3-nano-omni-30b-a3b-reasoning:free", name: "Nemotron Nano Omni", f: true,  p: "NVIDIA", d: "Omni gratuit" },
...
export function ChatInterface({ convId: initialConvId }: { convId?: string }) {
  ...
  useEffect(() => {
    if (!convId || !anonId) { setMessages([]); return; }
    fetch(`/api/messages?conversationId=${convId}`, { headers: { "x-anon-user-id": anonId } })
      .then(r => r.json()).then(d => { if (d.messages) setMessages(d.messages.map((m: any) => ({ id: m.id, role: m.role, content: m.content, model: m.model }))); }).catch(() => {});
  }, [convId, anonId]);
  ...
  useEffect(() => { const h = (e: MouseEvent) => { if (pickerRef.current && !pickerRef.current.contains(e.target as Node)) setShowModelPicker(false); }; document.addEventListener("mousedown", h); return () => document.removeEventListener("mousedown", h); }, []);
  ...
  function handleFiles(files: FileList) {
    ...
    for (let i = 0; i < files.length; i++) {
      ...
      if (f.size > 10485760) { toast.error(">10 MB"); continue; }
      if (images.length + newImages.length >= 5) { toast.error("Max 5 images"); break; }
  ...
  function handlePaste(e: React.ClipboardEvent) {
    const files: File[] = [];
    for (let i = 0; i < e.clipboardData.items.length; i++) { if (e.clipboardData.items[i].type.startsWith("image/")) { const file = e.clipboardData.items[i].getAsFile(); if (file) files.push(file); } }
    if (files.length) { const dt = new DataTransfer(); files.forEach(f => dt.items.add(f)); handleFiles(dt.files); }
  }
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    let cid = convId;
    if (!cid) { try { const res = await fetch("/api/conversations", { method: "POST", headers: { "Content-Type": "application/json", "x-anon-user-id": anonId }, body: JSON.stringify({ title: input.trim().slice(0, 60) || "Nouveau chat", model }) }); const data = await res.json(); if (data.conversation) { cid = data.conversation.id; setConvId(cid); window.dispatchEvent(new CustomEvent("elshalflow_refresh_convs")); window.history.pushState({}, "", `/chat?c=${cid}`); } } catch {} }

    let userContent: any;
    if (images.length > 0) {
      const parts: any[] = [];
      if (input.trim()) parts.push({ type: "text", text: input.trim() });
      const dataUrls = await Promise.all(images.map(img => fileToDataURL(img.file)));
      for (const dataUrl of dataUrls) { parts.push({ type: "image_url", image_url: { url: dataUrl } }); }
      userContent = parts;
    } else { userContent = input.trim(); }

    setMessages(prev => [...prev, { id: "u" + Date.now(), role: "user", content: input.trim() || "[Image]", images: images.map(i => i.preview), model }]);
    setInput(""); setImages([]); setLoading(true);

    const apiMessages = messages.map((m: any) => {
      if (m.images?.length) { const parts: any[] = []; if (typeof m.content === "string" && m.content !== "[Image]") parts.push({ type: "text", text: m.content }); for (const url of m.images) { parts.push({ type: "image_url", image_url: { url } }); } return { role: m.role, content: parts }; }
    ...
    try {
      const response = await fetch("/api/chat", { method: "POST", headers: { "Content-Type": "application/json", "x-anon-user-id": anonId }, body: JSON.stringify({ messages: apiMessages, model, conversationId: cid }), signal: abortRef.current.signal });
      ...
      function flushUI() {
        ...
        if (now - lastUIUpdate >= 30) {
          setMessages(prev => prev.map(m => m.id === assistantId ? { ...m, content: streamedContent, model: streamedModel } : m));
          lastUIUpdate = now;
        }
      }
      ...
      while (true) {
        ...
        while ((boundary = buffer.indexOf("\n\n")) !== -1) {
          ...
          const dataIdx = frame.lastIndexOf("data: ");
          if (dataIdx === -1) continue;
          try {
            const parsed = JSON.parse(frame.slice(dataIdx + 6));
            if (parsed.delta) { streamedContent += parsed.delta; if (parsed.model) streamedModel = parsed.model; flushUI(); }
      ...
      if (err.name !== "AbortError") { toast.error(err.message); setMessages(prev => prev.map(m => m.id === assistantId ? { ...m, content: `Erreur: ${err.message}` } : m)); }
  ...
  function renderMarkdown(text: string) {
    ...
    html = html.replace(/`([^`]+)`/g, '<code class="bg-secondary/50 px-1 py-0.5 rounded text-[11px]">$1</code>');
  ...
  const freeModels = MODELS.filter(m => m.f);
  const paidModels = MODELS.filter(m => !m.f);
  const filteredModels = MODELS.filter(m => { if (filterTab === "free" && !m.f) return false; if (filterTab === "paid" && m.f) return false; if (search && !m.name.toLowerCase().includes(search.toLowerCase()) && !m.p.toLowerCase().includes(search.toLowerCase())) return false; return true; });
      ...
      <div className="border-t border-border px-3 py-3">
        <div className="max-w-3xl mx-auto relative" ref={pickerRef}>
          ...
          <div className="mt-2 flex items-center gap-2 overflow-x-auto pb-1">
            ...
            {freeModels.slice(0, 6).map(m => (<button key={m.id} type="button" onClick={() => selectModel(m.id)} className={`text-[11px] px-2.5 py-1.5 rounded-full border whitespace-nowrap shrink-0 ${m.id === model ? "border-primary/30 bg-primary/10 text-primary" : "border-border text-muted-foreground hover:border-primary/20"}`}>{m.name}</button>))}
            <button type="button" onClick={() => { setShowModelPicker(true); setFilterTab("paid"); }} className="text-[11px] px-2.5 py-1.5 rounded-full border border-border text-muted-foreground whitespace-nowrap shrink-0">+{paidModels.length} payants</button>
          </div>
          {showModelPicker && (<div className="absolute bottom-full mb-2 left-0 right-0 bg-card border border-border rounded-xl shadow-2xl overflow-hidden z-50" style={{ maxHeight: "60vh" }}><div className="p-3 border-b border-border space-y-2"><input value={search} onChange={e => setSearch(e.target.value)} placeholder="Rechercher un modele..." className="w-full bg-secondary/50 rounded-lg px-3 py-2 text-xs outline-none" autoFocus /><div className="flex gap-1">{(["all", "free", "paid"] as const).map(tab => { const count = tab === "all" ? MODELS.length : tab === "free" ? freeModels.length : paidModels.length; const active = filterTab === tab; return (<button key={tab} type="button" onClick={() => setFilterTab(tab)} className={`text-[11px] px-2.5 py-1 rounded-md ${active ? tab === "free" ? "bg-green-500/20 text-green-400" : tab === "paid" ? "bg-amber-500/20 text-amber-400" : "bg-primary/20 text-primary" : "text-muted-foreground"}`}>{tab === "all" ? "Tous" : tab === "free" ? "Gratuits" : "Payants"} ({count})</button>); })}</div></div><div className="overflow-y-auto" style={{ maxHeight: "50vh" }}>{filteredModels.slice(0, 30).map(m => (<button key={m.id} type="button" onClick={() => selectModel(m.id)} className={`w-full text-left px-3 py-2.5 hover:bg-secondary/50 flex items-center justify-between gap-2 border-b border-border/20 ${m.id === model ? "bg-primary/5 border-l-2 border-l-primary" : ""}`}><div className="min-w-0"><div className="text-xs font-medium flex items-center gap-1.5">{m.name}{m.f ? <span className="text-[10px] bg-green-500/15 text-green-400 px-1 py-0 rounded">Gratuit</span> : <span className="text-[10px] bg-amber-500/15 text-amber-400 px-1 py-0 rounded">Payant</span>}</div><div className="text-[10px] text-muted-foreground truncate">{m.p} · {m.d}</div></div>{m.id === model && <div className="w-2 h-2 rounded-full bg-primary shrink-0" />}</button>))}</div></div>)}
          <p className="text-[10px] text-muted-foreground text-center mt-1.5">{hasKey ? "Cle OK · " : "Ajoutez cle dans Cles API · "}{freeModels.length} gratuits · {paidModels.length} payants</p>
        </div>
      </div>
    </div>
  );
